...
from Components.ServiceScanBlind import ServiceScanBlind as CScanBlind
...
class ServiceScanBlindSummary(Screen):
	skin = """
...
class ServiceScanBlind(Screen):
	...
	def __init__(self, session, scanList):
		...
		self.onFirstExecBegin.append(self.doServiceScanBlind)

	def doServiceScanBlind(self):
		self["scan"] = CScanBlind(self["scan_progress"], self["scan_state"], self["servicelist"], self["pass"], self.scanList, self["network"], self["transponder"], self["FrontendInfo"], self.session.summary)

	def createSummary(self):
		print "ServiceScanCreateSummary"
		return ServiceScanBlindSummary
