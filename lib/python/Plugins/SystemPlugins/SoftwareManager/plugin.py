...
class UpdatePluginMenu(Screen):
	...
	def __init__(self, session, args = 0):
		...
		if self.menu == 0:
			...
			self.list.append(("install-extensions", _("Manage extensions"), _("\nManage extensions or plugins for your Cuberevo" ) + self.oktext, None))
			self.list.append(("software-update", _("Software update"), _("\nOnline update of your Cuberevo software." ) + self.oktext, None))
			self.list.append(("software-restore", _("Software restore"), _("\nRestore your Cuberevo with a new firmware." ) + self.oktext, None))
			self.list.append(("system-backup", _("Backup system settings"), _("\nBackup your Cuberevo settings." ) + self.oktext + "\n\n" + self.infotext, None))
			self.list.append(("system-restore",_("Restore system settings"), _("\nRestore your Cuberevo settings." ) + self.oktext, None))
		...
		if current:
			...
			if self.menu == 0:
				if (currentEntry == "software-update"):
					self.session.openWithCallback(self.runUpgrade, MessageBox, _("Do you want to update your Cuberevo?")+"\n"+_("\nAfter pressing OK, please wait!"))
...
class PluginManager(Screen, DreamInfoHandler):
	...
	def runExecuteFinished(self):
		...
		if restartRequired:
			self.session.openWithCallback(self.ExecuteReboot, MessageBox, _("Install or remove finished.") +" "+_("Do you want to reboot your Cuberevo?"), MessageBox.TYPE_YESNO)
		else:
			self.selectedFiles = []
		...
		if self.cmdlist is not None:
			for entry in self.cmdlist:
				...
				if cmd == 0:
					...
					info = _("Cuberevo software because updates are available.")
...
class PluginDetails(Screen, DreamInfoHandler):
	...
	def runUpgradeFinished(self):
		...
		restartRequired = plugins.restartRequired
		if restartRequired:
			self.session.openWithCallback(self.UpgradeReboot, MessageBox, _("Installation finished.") +" "+_("Do you want to reboot your Cuberevo?"), MessageBox.TYPE_YESNO)
		else:
			self.close(True)
	...
	def checkNetworkCB(self,data):
		if data is not None:
			if data <= 2:
				...
				self.status.setText(_("Upgrading Cuberevo... Please wait"))
	...
	def ipkgCallback(self, event, param):
		if event == IpkgComponent.EVENT_DOWNLOAD:
			...
			if self.updating:
				...
				self.activityTimer.stop()
				self.activityslider.setValue(0)
				error = _("your Cuberevo might be unusable now. Please consult the manual for further assistance before rebooting your Cuberevo.")
				if self.packages == 0:
					error = _("No packages were upgraded yet. So you can check your network and try again.")
				if self.updating:
					error = _("Your Cuberevo isn't connected to the internet properly. Please check it and try again.")
	...
	def exit(self):
		if not self.ipkg.isRunning():
			if self.packages != 0 and self.error == 0:
				self.session.openWithCallback(self.exitAnswer, MessageBox, _("Upgrade finished.") +" "+_("Do you want to reboot your Cuberevo?"))
			else:
				self.close()
		else:
...
class PacketManager(Screen, NumericalTextInput):
	...
	def runRemoveFinished(self):
		self.session.openWithCallback(self.RemoveReboot, MessageBox, _("Remove finished.") +" "+_("Do you want to reboot your Cuberevo?"), MessageBox.TYPE_YESNO)
	...
	def runUpgradeFinished(self):
		self.session.openWithCallback(self.UpgradeReboot, MessageBox, _("Upgrade finished.") +" "+_("Do you want to reboot your Cuberevo?"), MessageBox.TYPE_YESNO)
