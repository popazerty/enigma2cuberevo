...
void * rcThread(void *vData)
{
    ...
    ilen = strlen(srvadr.sun_path) + sizeof(srvadr.sun_family);
    bind( srvfd, (struct sockaddr *) &srvadr, ilen );
    listen( srvfd, 10 );
    ...
    while (die == 0)
    {
       newKey = 0;
       RCCode = -1;
       if(recv(rc, &stmp, sizeof("00000000"), 0)>0)
       {
	   stmp[8]= '\0';
	   debugf(1, "TT_KEY->: %s\n", stmp);
	   sscanf(stmp, "%d", &key);
